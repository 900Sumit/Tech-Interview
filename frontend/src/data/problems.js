...
int main() {
    ...
    printArray(res1, size1); // Expected: [0,1]
    if(res1) free(res1);

    int nums2[] = {3, 2, 4};
    int size2;
    int* res2 = twoSum(nums2, 3, 6, &size2);
    printArray(res2, size2); // Expected: [1,2]
    if(res2) free(res2);

    int nums3[] = {3, 3};
    int size3;
    int* res3 = twoSum(nums3, 2, 6, &size3);
    printArray(res3, size3); // Expected: [0,1]
    if(res3) free(res3);
...
void printCharArr(char* s, int size) {
    printf("[");
    for(int i=0; i<size; i++) {
...
export const LANGUAGE_CONFIG = {
  ...
  c: {
    name: "C",
    icon: "/C.png", // Ensure you have this image in public folder
    monacoLang: "c",
  },
  cpp: {
    name: "C++",
    icon: "/cpp.png", // Ensure you have this image in public folder
    monacoLang: "cpp",
  },
};